...
namespace ARSoft.Tools.Net.Dns
{
	public abstract class DnsClientBase
	{
			...
			where TMessage : DnsMessageBase, new()
		{
			...
			PrepareMessage(message, out messageLength, out messageData);
			...
			for (int i = 0; i < endpointInfos.Count; i++)
			{
				...
				try
				{
					...
					if (resultData != null)
					{
						...
						try
						{
							result = DnsMessageBase.Parse<TMessage>(resultData);
						}
						catch (Exception e)
						{
						...
						if (result.IsTcpResendingRequested)
						{
							...
							if (resultData != null)
							{
								...
								try
								{
									tcpResult = DnsMessageBase.Parse<TMessage>(resultData);
								}
								catch (Exception e)
								{
									Trace.TraceError("Error on dns query: " + e);
			...
			where TMessage : DnsMessageBase, new()
		{
			...
			for (int i = 0; i < endpointInfos.Count; i++)
			{
				...
				try
				{
					...
					try
					{
						result = DnsMessageBase.Parse<TMessage>(resultData.Buffer);
					}
					catch (Exception e)
					{
						Trace.TraceError("Error on dns query: " + e);
					...
					if (result.IsTcpResendingRequested)
					{
						...
						if (resultData != null)
						{
							...
							try
							{
								tcpResult = DnsMessageBase.Parse<TMessage>(resultData.Buffer);
							}
							catch (Exception e)
							{
								Trace.TraceError("Error on dns query: " + e);
			...
			where TMessage : DnsMessageBase, new()
		{
			...
			GetEndpointInfos().Select(x => SendMessageParallelAsync(x, message, messageData, messageLength, results, CancellationTokenSource.CreateLinkedTokenSource(token, cancellationTokenSource.Token).Token)).ToArray();
		...
		private async Task SendMessageParallelAsync<TMessage>(DnsClientEndpointInfo endpointInfo, TMessage message, byte[] messageData, int messageLength, BlockingCollection<TMessage> results, CancellationToken token)
			where TMessage : DnsMessageBase, new()
		{
			using (UdpClient udpClient = new UdpClient(new IPEndPoint(endpointInfo.LocalAddress, 0)))
			{
				...
				while (true)
				{
					...
					try
					{
						result = DnsMessageBase.Parse<TMessage>(response.Buffer);
					}
					catch (Exception e)
					{
						Trace.TraceError("Error on dns query: " + e);
